...
The harness is built and has been run, and the baseline won. Measured over 72 runs on 2026-09-06: full context reached every gold provision, search alone reached 0.41 and 0.46 of them, and search plus traversal 0.54 and 0.56. Retrieval's case on this corpus is cost and checkability rather than accuracy. `docs/evaluation.md` carries the numbers and `python/data/eval/results.json` carries the run they were rendered from, including the provision ids each arm searched, traversed and dropped on every question. This entry keeps the decision and that document keeps the numbers, so a figure moves in one place.
