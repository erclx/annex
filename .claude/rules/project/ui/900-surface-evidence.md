...
- Write the shipped state to `web/ui-evidence/<state>/`, one file per theme, replacing the file that is there rather than adding beside it.
...
- Read the design source before capturing, being `.claude/wireframes/answer.md` for layout and copy, `.claude/DESIGN.md` for tokens, and `web/ui-evidence/settled-design.html` for the type scale, spacing and treatments the drawn design settled.
...
- Link `web/ui-evidence/` from the pull request body whenever the diff changes what the surface renders. A reviewer who cannot see the surface is reviewing the diff alone.
