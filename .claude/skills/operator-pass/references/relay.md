...
Read when the operator calls a batch, and at the start of the pass when a launch brief asks for collisions.

## Where the batch goes

- Send it to the controller where the session was dispatched by one, resolved per `.claude/rules/canon/core/091-channel.md`.
- Give it to the operator as one list in the conversation where no controller exists. The pass file is the handoff either way.
