...
- **Build the vector index: `cd python && uv run python -m annex embed`.** It needs Ollama up and writes the gitignored `python/data/index/`. A fresh clone or a new worktree has no index and `search` says so rather than failing on a missing table. It took 10.6 seconds warm rather than the few minutes this step used to claim, and the retrieval entry carries the conditions that figure holds under
