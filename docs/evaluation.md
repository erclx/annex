...
**Latency.** Measured throughout and engineered nowhere. Every wall time in this report is real, and none of it has been optimized. The page streams each step as it finishes, so a reader watches search, the walk and the budget cut land rather than a blank wait, but the answer itself still arrives whole at the end of drafting. There is no answer cache, so the same description costs the same twice. And the routing step spends a model call restating a question that is sometimes already in the Act's vocabulary. Streaming changed what the wait shows and not how long it takes, since no step here runs faster for being reported.
