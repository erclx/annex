...
- **`--question` narrows which pair is asked, and still writes the whole manifest.** Run against `web/src/fixtures/` itself, a one-question capture overwrites `manifest.json` and `index.ts` with only the entries the invocation named, wiping the other 23. `git checkout -- manifest.json index.ts` after the run repairs it whenever the recaptured entry's own fields (`question_id`, `description`, `flow`, `version`, `file`, `refused`) are unchanged from before, since neither file carries a per-fixture stamp for when one entry's content moved. Measured on `feature-recapture-and-correct-the-evidence-readme` on 2026-09-14.
...
- **The root gate no longer needs `web/out-replay/` and `web/test-results/` removed by hand first.** `bun run check` runs prettier at the root with `.gitignore` and `.prettierignore` as its ignore paths, and `web/.gitignore` alone held both folders, so a replay export left in the tree failed `check:format`, and so did the `error-context.md` files a failing Playwright run writes under `web/test-results/`. `.prettierignore` now carries both beside `web/.next/`. A full `bun run test:e2e` followed by `./scripts/verify.sh` in the same tree passes with nothing removed. `cspell.json` never carried the gap: `useGitignore` reads every nested `.gitignore` rather than one file, so it already skipped both folders. Measured at this branch on 2026-09-14.
- **A repository-root `tsconfig.json` is the third instance of this shape: a tool reaching files it was never meant to read.** `bun init` scaffolds one at the root, and it carried no `include`, `exclude`, or `paths`, so `tsc --listFilesOnly` against it claimed every file `web/tsconfig.json` also claims, with none of that project's settings: no `@/*` alias, and `types: ["bun"]` alone left a React callback parameter with no DOM types to infer from. An editor that attached a `web/` file to the root project reported both errors on code `tsc` compiles clean, which is how this was found. Zero tracked TypeScript files live outside `web/`, and `canon tooling sync web . --check` lists the file under neither configs nor seeds, so deleting it is not undone by the next sync. The file is removed rather than scoped with an `include` that matches nothing, joining `out/` reaching eslint as source and the root gate reformatting a browser run's output as the two siblings this shape already had. Measured on `feature-scope-the-root-typescript-project` on 2026-09-14.
