...
- **Isolating one flaky Playwright test does not reproduce a CI-only race.** Running only the failing test in a fresh worker passed ten times in a row, where running the whole spec file, or the full e2e suite, repeatedly under `taskset -c 0-3` with `CI=1` (which disables `reuseExistingServer`) reproduced the same failure on roughly half the runs. The contention the flake needs comes from the surrounding suite, not from the target test alone, so a reproduction that isolates the test first reports a false negative. Measured 2026-09-15.
