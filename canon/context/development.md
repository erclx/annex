...
- **Remove `web/out-replay/` and `web/test-results/` before running the root gate.** `bun run check` runs prettier at the root with the root `.gitignore` as its only ignore path, and `web/.gitignore` alone ignores both folders. A replay export left in the tree is reformatted and fails `check:format`, and so are the `error-context.md` files a failing Playwright run writes under `web/test-results/`.
