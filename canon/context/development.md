...
- **The replay build takes seconds to answer a pick.** It plays the recording's steps at a fifth of the recording's `duration_ms` before the answer renders, up to about 17 seconds, so `web/playwright.config.ts` raises the expect timeout to 20 seconds and a replay case waiting on an answer needs no timeout of its own.
