...
Both folders divide by the surface region and the data source a file serves rather than by any naming convention, read off `canon/wireframes/answer.md`'s own regions. A test file sits beside the source it tests.
...
The recorded-question copy on `web/src/lib/service/replay.test.ts` couples to the fixture count: the wireframe's supporting text types the word `Twelve`, and the test fails when the recording stops holding twelve questions, so a re-capture that changes the count cannot ship the old number.
...
- **It costs what an evaluation arm costs, not what a full run costs.** Twenty-four pipeline calls, one per question per version, run roughly 42 seconds an answer with models already resident, fifteen of twenty-four fixtures written ten minutes in and the full set closer to 17 minutes than to ten, against the tens of minutes `evaluate` takes over three arms. Measured on `feat/draw-the-traversal` on 2026-09-12.
...
- **Next 16 refuses a second `next dev` out of one project directory whatever port it is given.** It answers `Another next dev server is already running` and names the first server's PID. The end-to-end run needs two builds of the web half, since the replay flag is read at module scope, so `playwright.config.ts` serves the replay one as a static export behind `web/e2e/serve-export.ts` rather than as a second dev server. That script resolves `/ask` to `ask.html` the way Cloudflare Pages does, where `python3 -m http.server` answers `/ask` with a 404, and its docstring carries the rest of the reasoning. That costs a production build at the head of the run and buys a test against the artifact the deploy uploads rather than against a development stand-in.
...
- **`.prettierignore` carries `web/out-replay/`, `web/test-results/` and `web/.next/`.** `bun run check` runs prettier at the root with `.gitignore` and `.prettierignore` as its ignore paths, so a replay export left in the tree and the `error-context.md` files a failing Playwright run writes under `web/test-results/` do not fail `check:format`. `cspell.json` needs no equivalent entry: `useGitignore` reads every nested `.gitignore` rather than one file, so it already skips both folders.
- **Zero tracked TypeScript files live outside `web/`, so no root `tsconfig.json` should exist.** `bun init` scaffolds one at the root with no `include`, `exclude`, or `paths`, which makes `tsc --listFilesOnly` against it claim every file `web/tsconfig.json` also claims, with none of that project's settings: no `@/*` alias, and a callback parameter left with no DOM types to infer from since `types` holds only `["bun"]`. Delete such a file rather than scope it with an `include` that would match nothing. `canon tooling sync web . --check` lists it under neither configs nor seeds, so removing it is not undone by the next sync. Measured at `c7abe4d` on 2026-09-14.
- **Wait for the docked region before measuring layout in an e2e case.** `useDocked` starts false and flips once the media query reads, which moves `BeforeYouAsk` from the single column into the docked pane. A locator resolved before that flip evaluates an unmounted node, so every box and computed style on it reads empty. Read grid placement off `getComputedStyle` rather than the `style` attribute as well, since the server writes an inline style with no space after the colon, as `grid-column:2`, and hydration leaves the attribute as written.
...
- **Rebuild with `rm -rf .next` between a replay build and a local build sharing one `web/` directory.** `NEXT_PUBLIC_ANNEX_MODE=replay ANNEX_DIST_DIR=out-replay` leaves `.next` holding the replay build, so building the local half afterward without clearing `.next` serves the replay page from `bunx next start`, and `capture-states.ts` run against it captures the replay page's states instead of the local page's. Measured at `b93e8be` on 2026-09-14.
...
- **A Server Component page cannot pass an inline function prop to a Client Component it renders, even a no-op.** The RSC boundary refuses any function value regardless of whether it is ever called, so `web/src/app/evaluation/page.tsx` needed `'use client'` solely because it renders `TopBar`, which always takes event-handler props such as `onTraversalChange`. `next build` catches this while exporting the page with "Event handlers cannot be passed to Client Component props", not at typecheck or lint. Measured on `feature-evaluation-route` on 2026-09-14.
- **Reproducing a CI-only race takes repeated runs, and what surfaces it differs by tool.** A Vitest race needed cross-test contention: the single test alone, run eight times under a single-threaded pool, never failed, where the whole spec file run three times under `taskset -c 0-3` reproduced it once. A Playwright race needed no contention at all: the single test alone, filtered with `-g` and run under `CI=1` (which disables `reuseExistingServer`), failed on roughly half of eight repeated runs by itself. Neither race showed on one run of either kind. Measured 2026-09-15.
